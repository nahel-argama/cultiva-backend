...
HTTP authentication messages (historical): an earlier intermediate test for `tests/Feature/bootstrap/AppTest.php` was removed because it covered Laravel defaults rather than custom application behavior. Authentication is covered by the real endpoint Feature tests.
...
Native pagination revision (historical): `docker compose exec php vendor/bin/phpunit tests/Feature/domain/Models/Offer/Http/Controllers/OfferController/IndexTest.php tests/Feature/domain/Models/Offer/Http/Controllers/OfferController/AvailableIndexTest.php` produced valid RED with 13 tests, 24 assertions and 3 failures. The intermediate paginator implementation and its custom response/request classes were superseded by the current Laravel Resource collection.
...
Final boolean pagination contract (historical): the focused command produced valid RED while the six-field contract was still being evaluated. That intermediate contract was subsequently replaced by Laravel's native Resource pagination.

Final regression for boolean navigation (historical): the suite was GREEN at that intermediate revision. The current code is validated separately below.

Current documentation/code alignment: Offer and Purchase list endpoints return Laravel `Resource::collection()` pagination with `data`, `links` and `meta`; categories are inserted by migration; undeclared offer fields are ignored; and the current full suite is GREEN with 108 tests and 388 assertions.
